...
public class ChangesTokenDefinitions : TokenDefinitionBase<ChangesTokenType>
{
    public ChangesTokenDefinitions()
    {
        ...
        addToken(ChangesTokenType.Stop, "(?<=\n\\s*)[а-я0-9]+([).])\\s*(в\\s+)?(част|стать|пункт|подпун|разд|глав|абзац|правилах)");
        addToken(ChangesTokenType.Stop, "(?<=\n\\s*)[а-я0-9]+([).])\\s*[(]");
        addToken(ChangesTokenType.Stop, "(?<=\n\\s*)[а-я0-9]+([).])\\s*после\\s+(част|стать|пункт|подпун|разд|глав)");
        addToken(ChangesTokenType.Stop, "(?<=\n\\s*)[а-я0-9]+([).])\\s*наименование\\s+(част|стать|пункт|подпун|разд|глав)");
