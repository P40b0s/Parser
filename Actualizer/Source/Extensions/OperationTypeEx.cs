...
{
    public static OperationType GetElementOperationType(this Operation op, List<Token<ActualizerTokenType>> tokens)
    {
        ...
        if(tokens.Any(a=>a.TokenType == ActualizerTokenType.OperationUnitWord) && tokens.Any(a=>a.TokenType == ActualizerTokenType.Add) && tokens.Any(a=>a.TokenType == ActualizerTokenType.After))
    ...
    public static IEnumerable<OperationType> GetWordOperationType(this Operation op, List<Token<ActualizerTokenType>> tokens)
    {
        //после слов "абырвалг" дополнить словани "- Главрыба"
        if(tokens.Any(a=>a.TokenType == ActualizerTokenType.OperationUnitWord) && tokens.Any(a=>a.TokenType == ActualizerTokenType.Add) && tokens.Any(a=>a.TokenType == ActualizerTokenType.After))
            yield return OperationType.ApplyAfterWords;
        //слова "Эваывацуа" удалить
        if(tokens.Any(a=>a.TokenType == ActualizerTokenType.OperationUnitWord) && tokens.Any(a=>a.TokenType == ActualizerTokenType.Remove))
            yield return OperationType.RemoveWord;
        //слова "ваоывоавта" заменить....
        if(tokens.Any(a=>a.TokenType == ActualizerTokenType.OperationUnitWord) && tokens.Any(a=>a.TokenType == ActualizerTokenType.Replace))
            yield return OperationType.ReplaceWords;
        //Дополнить словами "абырвалг"
        if(tokens.Any(a=>a.TokenType == ActualizerTokenType.OperationUnitWord) && tokens.Any(a=>a.TokenType == ActualizerTokenType.Add))
            yield return OperationType.ApplyWordsToEnd;
    }
}
